...
<script>
// Send a credentialed XHR with garbage credentials to flush the browser's
// Basic Auth cache for this origin, then navigate to "/" to re-prompt.
try {
  var x = new XMLHttpRequest();
  x.open('GET', '/', false, '__logout__', '__' + Math.random());
  x.send();
} catch(e) {}
window.location.replace('/');
</script>
<noscript><a href="/" style="color:#388bfd">Log in</a></noscript>
</body></html>`))
}
